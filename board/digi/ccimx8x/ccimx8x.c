...
void generate_partition_table(void)
{
	...
	if (capacity_gb >= 15)
		env_set("parts_linux", LINUX_16GB_PARTITION_TABLE);
	else if (capacity_gb >= 7)
		env_set("parts_linux", LINUX_8GB_PARTITION_TABLE);
	else
		env_set("parts_linux", LINUX_4GB_PARTITION_TABLE);
}
